...
class Player:
    ...
    def move(self, room, current_screen, difficulty):
        ...
        if difficulty <= 1:
            if (room.collision(self.x + 1, self.y + 1) == 2
                or room.collision(self.x + 6, self.y + 1) == 2
                or room.collision(self.x + 1, self.y + 6) == 2
                or room.collision(self.x + 6, self.y + 6) == 2
            ):
                self.alive = 0
        else:
            if (room.collision(self.x, self.y) == 2
                or room.collision(self.x + 7, self.y) == 2
                or room.collision(self.x, self.y + 7) == 2
                or room.collision(self.x + 7, self.y + 7) == 2
            ):
                self.alive = 0
        
        if (room.collision(self.x, self.y) == 3
            or room.collision(self.x + 7, self.y) == 3
            or room.collision(self.x, self.y + 7) == 3
            or room.collision(self.x + 7, self.y + 7) == 3
        ):
            self.win = 1
